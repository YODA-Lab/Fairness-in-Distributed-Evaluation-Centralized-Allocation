...
save_path = "logs/SOTO/alpha_fair/"
...
alpha_fairness = fairness_function.alpha
config = {"agent":
		  {
...
for i_episode in range(1, args.n_episode+1):
	...
	for steps in range(1, args.max_steps+1):
		...
		if steps % T == 0:
			...
			for i in range(n_agent):
				if not greedy[i]:
					Pi[i].update(ep_states[i], ep_actions[i], all_ep_advantages_normed, ep_masks[i])
				else:
					gPi[i].update(ep_states[i], ep_actions[i], all_ep_advantages[i], ep_masks[i])
