...
if shared_nets:
	#sharing the network
	meta_Pi_shared = PPOPolicyNetwork(num_features=num_features+2, num_actions=n_signal, layer_size=128, epsilon=0.1, learning_rate=lr_actor)
	...
	for i in range(n_agent):
		for j in range(n_signal):
			Pi_shared[i].append(PPOPolicyNetwork(num_features=num_features, num_actions=M.n_actions, layer_size=256, epsilon=0.1, learning_rate=lr_actor))
	...
	for i in range(n_agent):
		meta_Pi.append(PPOPolicyNetwork(num_features=num_features+2, num_actions=n_signal, layer_size=128, epsilon=0.1, learning_rate=lr_actor))
		meta_V.append(ValueNetwork(num_features=num_features+2, hidden_size=128, learning_rate=0.001))
		for j in range(n_signal):
			Pi[i].append(PPOPolicyNetwork(num_features=num_features, num_actions=M.n_actions, layer_size=256, epsilon=0.1, learning_rate=lr_actor))
...
for i_episode in range(1, args.n_episode+1):
	...
	for steps in range(1, args.max_steps+1):
		...
		# Allocate the actions assuming the policy is q values
		...
		for i in range(n_agent):
			# u[i].append(rewards[i]) # Change in validation as well
...
if args.training:
	...
	results_file = f"Results/FEN-ILP/{args.env_name+args.env_name_mod}results.csv"
	# Also save a copy of the results file in the save_path
	results_file2 = f"{args.save_path}/results.csv"
	create=False
	if not os.path.exists("Results/FEN-ILP"):
		os.makedirs("Results/FEN-ILP")
