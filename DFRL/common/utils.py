...
def make_env(config, normalize_inputs):
    ...
    if env_name == 'matthew':
        ...
        env = Env(normalize_inputs, int(config['agent']['sumo_T']), str2bool(config['agent']['sumo_doublereward']))
    elif env_name == 'iroko':
        from common.env_iroko import Env
        env = Env(normalize_inputs, int(config['agent']['iroko_T']), float(config['agent']['iroko_gamma']),
                  str2bool(config['agent']['iroko_more_obs']), str2bool(config['agent']['iroko_average_rewards']))
    elif env_name == 'pendulum':
        from common.env_pendulum import Env
        env = Env(normalize_inputs)
    else:
        raise("Not supported")
    return env
...
def default_config():
    ...
    config['agent']['env_name'] = 'matthew'
    config['agent']['resource'] = 'all' #in (all, rr, nrr)

    #env
    config['agent']['matthew_neighbors_obs'] = 'true'
    config['agent']['sumo_T'] = '50'
    config['agent']['sumo_doublereward'] = 'false'
    config['agent']['plant_T'] = '50'
    config['agent']['plant_ohe'] = 'true'
    config['agent']['iroko_T'] = '50'
    config['agent']['iroko_more_obs'] = 'false'
    config['agent']['iroko_average_rewards'] = 'true'
    config['agent']['iroko_gamma'] = '0.98'
    ...
    #decaf
    config['agent']['learning_beta'] = '0.9'
    config['agent']['fairness_type'] = 'variance'
    config['agent']['f_rewards_scale'] = '1'
...
def default_tensorflow():
    ...
    config = tf.compat.v1.ConfigProto(intra_op_parallelism_threads=1, inter_op_parallelism_threads=1, allow_soft_placement=True,
                        device_count={'CPU': 1})
    # config.gpu_options.allow_growth=True   
