...
save_path = "logs/SOTO-ILP/alpha_fair/"
...
args.fairness_type = 'alpha_fair'
fairness_function = fairness_router(args.fairness_type)
alpha_fairness = fairness_function.alpha # SOTO paper uses 0.9
config = {"agent":
		  {
	...
	for steps in range(1, args.max_steps+1):
		...
		if steps % T == 0:
			...
			for i in range(n_agent):
				...
				all_ep_advantages_unnormed.append(ep_advantages)
			...
			all_ep_advantages_unnormed = np.array(all_ep_advantages_unnormed)

			all_ep_advantages_unnormed = ((average_jpi+0.0000000001)**-alpha_fairness) @ all_ep_advantages_unnormed
			all_ep_advantages_normed = (all_ep_advantages_unnormed - np.mean(all_ep_advantages_unnormed)) / (np.std(all_ep_advantages_unnormed) + 0.0000000001)
			for i in range(n_agent):
				if not greedy[i]:
					Pi[i].update(ep_states[i], ep_actions[i], all_ep_advantages_normed)
				else:
					gPi[i].update(ep_states[i], ep_actions[i], all_ep_advantages[i])
